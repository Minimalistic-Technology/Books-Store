...
export default function OrderList({ orders, cancelReasons, onEdit, onDelete, onCancel }: OrderListProps) {
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      ...
      setErrors((prev) => ({ ...prev, [id]: err.message || "Failed to delete order. Please try again." }));
    } finally {
  ...
  const handleCancel = (id: string) => {
    const reason = selectedReasons[id] === "Other" ? customReasons[id] || "" : selectedReasons[id];
    if (!reason) {
      setErrors((prev) => ({ ...prev, [id]: "Please select or enter a cancellation reason." }));
      return;
    }
    onCancel(id, reason);
    setErrors((prev) => ({ ...prev, [id]: "" }));
            ...
            <div key={order.id} className="bg-white p-4 rounded-lg shadow-md flex items-center justify-between">
              <div className="flex items-center space-x-4">
                ...
                <div>
                  ...
                  {order.cancelReason && (
                    <p className="text-sm text-gray-600">Cancel Reason: {order.cancelReason}</p>
                  )}
                </div>
              </div>
                ...
                {(order.status === "Processing" || order.status === "Shipped") && (
                  <div className="flex flex-col space-y-2">
                    <select
                      ...
                      onChange={(e) =>
                        setSelectedReasons((prev) => ({ ...prev, [order.id]: e.target.value }))
                      }
                    ...
                    <button
                      ...
                      disabled={!selectedReasons[order.id] || (selectedReasons[order.id] === "Other" && !customReasons[order.id])}
                      className={`px-3 py-1 rounded-lg text-white transition-all ${
                        !selectedReasons[order.id] || (selectedReasons[order.id] === "Other" && !customReasons[order.id])
                ...
                <div className="flex space-x-2">
                  ...
                  <button
                    ...
                    className={`bg-red-500 text-white px-3 py-1 rounded-lg transition-all ${
                      deletingOrderId === order.id ? "opacity-50 cursor-not-allowed" : "hover:bg-red-600"
                    }`}
                  >
                    Delete
                  </button>
